...
<?php
  ...
  $render_queue_lessons = Lesson::get_lessons_for_render_queue();
  $queuable_lessons = [];
  $queued_lessons = [];
  
  // Determine Queuable Lessons
  
  // Not queued
  // Past exportable threshold
  // No unfixed issues
  ...
  // Determine queued lessons
  
  // Queued
  
  foreach($render_queue_lessons as $lesson) {
    if($lesson->is_queued) {
      $queued_lessons[] = $lesson;
    }
  }
		...
		<table>
			<tr><th>Lesson</th><th>Actions</th><th>Last Action</th><th>Action Time</th><th>Last Render</th><th>Due Date</th></tr>
				...
				if(!$queuable_lessons) {
					...
					foreach($queuable_lessons as $lesson) {
              ...
              $last_issue = Issue::find_last_fixed_issue_for_lesson($lesson->id);
      				...
      				if($last_task->completed_time > $lesson->exported_time) {
      				  ...
      				  echo "<tr>";
    						...
    						echo "</td>";
    						echo "<td><form action='render-queue-yannick.php' method='post'>";
	...
	<div id="export-queue">
		...
		<table>
            ...
            echo "</td>";
						...
						echo "<form action='render-queue-yannick.php' method='post'><input type='hidden' name='qa_lesson_id' value='{$lesson->id}'><input type='submit' name='unqueue_lesson' value='Unqueue!'></form>";
						echo "</tr>";
					} 
				 } ?>		
		</table>
